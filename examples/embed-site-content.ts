      ...
      for (const word of words) {
        ...
        if (currentLength >= CHUNK_SIZE) {
          ...
          const overlapWords = Math.floor(
            currentChunk.length * (CHUNK_OVERLAP / CHUNK_SIZE),
          );
...
async function embedSiteContent() {
  ...
  console.log(
    `Created ${chunks.length} chunks from ${SITE_CONTENT.length} pages`,
  );
  ...
  if (deleteError) {
    console.error(
      "Warning: Could not clear existing embeddings:",
      deleteError.message,
    );
  }
  ...
  for (let i = 0; i < records.length; i += insertBatchSize) {
    ...
    console.log(
      `  Inserted ${Math.min(i + insertBatchSize, records.length)}/${records.length} records`,
    );
  }
